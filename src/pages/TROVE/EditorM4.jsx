...
export const EditableRowM4 = ({
  ...
  useEffect(() => {
    setEditedAmountF((editedAmountF.sub(troveCollateral)).toString(4));
  }, [editedAmountF]);
  useEffect(() => {
  ...
  useEffect(() => {
    setAmountF((amountF.sub(troveCollateral)).prettify(4));
  }, [amountF]);
  ...
  return editing === inputId ? (
    <RowM4 {...{ label, labelFor: inputId, unit }}>
      <Input
        ...
        onChange={e => {
          try {
            setEditedAmount(Decimal.from(e.target.value).add(troveCollateral));
            setInvalid(false);
          } catch {
            setInvalid(true);
          }
      ...
      >
        {maxAmount && (
          <Button
            ...
            style={{fontSize: "10px", padding: 0, paddingRight: "5px", opacity: "0.8", color: "#6B7280", backgroundColor: "transparent", border: "0", position: "absolute", right: 10}}
            onClick={event => {
              setEditedAmount(accountBalance.add(troveCollateral));
              event.stopPropagation();
            }}
            disabled={maxedOut} className="fontCustom-Medium"
          >
            MAX
          </Button>
        )}
