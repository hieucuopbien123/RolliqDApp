...
export const EditableRowM5 = ({
  ...
  useEffect(() => {
    setAmountF(suppliedAmount.sub(amountF).prettify());
  }, [amountF]);
  useEffect(() => {
    setEditedAmountF(suppliedAmount.sub(editedAmountF).toString(2));
  }, [editedAmountF]);
        ...
        editing === inputId ? (
          <RowM5 {...{ label, labelFor: inputId, unit }}>
            <Input
              ...
              onChange={e => {
                try {
                  setEditedAmount(suppliedAmount.sub(e.target.value));
                  setInvalid(false);
                } catch {
                  setInvalid(true);
                }
