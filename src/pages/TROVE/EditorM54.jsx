...
export const EditableRowM5 = ({
  ...
  useEffect(() => {
    setAmountF((amountF.sub(amountDeposited)).prettify());
  }, [amountF]);
  useEffect(() => {
    setEditedAmountF(editedAmountF.sub(amountDeposited).toString(2));
  }, [editedAmountF]);
        ...
        editing === inputId ? (
          <RowM5 {...{ label, labelFor: inputId, unit }}>
            <Input
              ...
              onChange={e => {
                try {
                  setEditedAmount(amountDeposited.add(e.target.value));
                  setInvalid(false);
                } catch {
                  setInvalid(true);
                }
