...
export const EditableRowM4 = ({
  ...
  useEffect(() => {
    setEditedAmountF(editedAmountF.sub(netdebt).toString(2));
  }, [editedAmountF]);
  // useEffect(() => {
  ...
  // }, [maxedOutF]);
  useEffect(() => {
    setAmountF(editedAmountF.sub(netdebt).prettify());
  }, [amountF]);
  ...
  return editing === inputId ? (
    <RowM4 {...{ label, labelFor: inputId, unit }}>
      <Input
        ...
        onChange={e => {
          try {
            setEditedAmount(Decimal.from(e.target.value).add(netdebt));
            setInvalid(false);
          } catch {
            setInvalid(true);
          }
      ...
      >
        {maxAmount && (
          <Button
            ...
            style={{fontSize: "small", padding: 0, paddingRight: "5px", opacity: "0.8", color: "#6B7280", backgroundColor: "transparent", border: "0"}}
            onClick={event => {
