  ...
  return (
    <>
      ...
      <FlexC justifyContent="space-between" alignItems="flex-end">
        <Text className="text-[#1E2185] fontCustom-bold text-[16px]">Supply ETH</Text>
      ...
      <Box sx={{ pt: [3, 4] }}>
        ...
        {stableTroveChange ? (
          <TroveActionM1
            ...
            maxBorrowingRate={maxBorrowingRate}
            borrowingFeeDecayToleranceMinutes={60}
            topmessage={`Supplying ${collateral.sub(trove.collateral).prettify(2)} ETH`}
          >
            Supply
          </TroveActionM1>
        ) : (
          <Button disabled className="w-full fontCustom-bold text-[18px] animationCustom" style={{backgroundColor: "#1E2185", borderRadius: "25px", padding: "7px"}}>Supply</Button>
        )}
      </Box>
    </>
  );
