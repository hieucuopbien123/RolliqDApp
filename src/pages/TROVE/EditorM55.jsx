...
export const EditableRowM5 = ({
  ...
  useEffect(() => {
    setAmountF(amountF.sub(suppliedAmount).prettify());
  }, [amountF]);
  useEffect(() => {
    setEditedAmountF(editedAmountF.sub(suppliedAmount).toString(2));
  }, [editedAmountF]);
        ...
        editing === inputId ? (
          <RowM5 {...{ label, labelFor: inputId, unit }}>
            <Input
              ...
              onChange={e => {
                try {
                  setEditedAmount(suppliedAmount.add(e.target.value));
                  setInvalid(false);
                } catch {
                  setInvalid(true);
                }
