...
import { ActionDescriptionM1, Amount } from "../../STAKING/ActionDescriptionM1";
...
const TroveChangeDescription = ({ params }) => (
  <>
    <ActionDescriptionM1>
      {params.depositCollateral && params.borrowRUSD ? (
        <>
          You will deposit <Amount>{params.depositCollateral.prettify()} ETH</Amount> and receive{" "}
          <Amount>
            {params.borrowRUSD.prettify()} {COIN}
          </Amount>
        </>
      ) : params.repayRUSD && params.withdrawCollateral ? (
        <>
          You will pay{" "}
          <Amount>
            {params.repayRUSD.prettify()} {COIN}
          </Amount>{" "}
          and receive <Amount>{params.withdrawCollateral.prettify()} ETH</Amount>
        </>
      ) : params.depositCollateral && params.repayRUSD ? (
        <>
      ...
      ) : params.borrowRUSD && params.withdrawCollateral ? (
        <>
...
export const selectForTroveChangeValidation = ({
  ...
  rusdBalance,
  numberOfTroves
}) => ({ price, total, accountBalance, rusdBalance, numberOfTroves });
  ...
  if (change.type === "invalidCreation") {
    ...
    return [
      ...
      <ErrorDescription>
        ...
        <Amount>
          {RUSD_MINIMUM_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    ];
...
const validateTroveCreation = (
  ...
  if (borrowRUSD.lt(RUSD_MINIMUM_NET_DEBT)) {
    return (
      <ErrorDescription>
        You must borrow at least{" "}
        <Amount>
          {RUSD_MINIMUM_NET_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    );
...
const validateTroveAdjustment = (
  { depositCollateral, withdrawCollateral, borrowRUSD, repayRUSD },
  {
    originalTrove,
    resultingTrove,
    recoveryMode,
  ...
  if (repayRUSD) {
    if (resultingTrove.debt.lt(RUSD_MINIMUM_DEBT)) {
      return (
        <ErrorDescription>
          Total debt must be at least{" "}
          <Amount>
    ...
    if (repayRUSD.gt(rusdBalance)) {
      return (
        <ErrorDescription>
          The amount you're trying to repay exceeds your balance by{" "}
          <Amount>
            {repayRUSD.sub(rusdBalance).prettify()} {COIN}
          </Amount>
          .
        </ErrorDescription>
      );
